...
const (
	...
	ControllerName = "tep-automation-controller"
	// Kind is used in the TaskRef for Runs using the TEP automation
	Kind = "TEPAutomation"
)

// NewController instantiates a new controller.Impl from knative.dev/pkg/controller
func NewController(ghToken string, performerFuncs ...func(*ghclient.TEPGHClient) performers.Performer) func(context.Context, configmap.Watcher) *controller.Impl {
	return func(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
		logger := logging.FromContext(ctx)
		runInformer := runinformer.Get(ctx)

		r := NewReconciler(ctx, ghToken, performerFuncs...)
